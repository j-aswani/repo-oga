...
from aggregator.utils.utils import Singleton, get_key, get_cleaned_data, \
...
class TestSingleton(AggregatorTestCase):
...
class TestDict(AggregatorTestCase):
...
class TestCensorer(AggregatorTestCase):
...
class TestString(AggregatorTestCase):
...
class TestDate(AggregatorTestCase):
